...
layui.define(['element'], function(exports){
   var  element = layui.element(),
        ...
        LarryTab.prototype.tabAdd = function(data){
		    ...
		    if(tabIndex === -1){
		    	globalTabIdIndex++;
		    	var content = '<div id="' +data.code+ '" src="' + data.href + '" data-id="' + globalTabIdIndex + '" class="larry-iframe"></div>';
			    var title = '';
			    // 若icon有定义
			    ...
			    //alert(data.code);
			    $.post(
					data.href+"?sit="+data.code,
					{page:1},
					function(data1){
						$("#"+data.code).html(data1);
					},"html"
				)

			/*	$.post(data.href,{page:1}, function(data2){
				  layer.open({
				    type: 1,
				    content: data2 //注意，如果str是object，那么需要字符拼接。
				  });
				});*/


			    //iframe 自适应
			    ELEM.contentBox.find('#'+data.code+'[data-id=' + globalTabIdIndex + ']').each(function() {
