...
// 			'oid_partner' => '201408071000001546',
			'oid_partner' => '201412101000127502',
			//安全检验码，以数字和字母组成的字符
			'key' => 'www.wjtrw.com',
	...
	//服务器异步通知页面路径
	private $notify_url = "http://www.wjtr.com/index.php?module=default&submodule=order&controller=payment&action=notify";
	//需http://格式的完整路径，不能加?id=123这类自定义参数
	

	//页面跳转同步通知页面路径
	private $return_url = "http://www.wjtr.com/index.php?module=default&submodule=order&controller=payment&action=response";
	...
	static function init($config) {
		...
		if (null == self::$_app) {
			self::$_app = new Helper_Llpay($config);
		}
	...
	function response() {
		...
		if (empty($_POST)) { //判断POST来的数组是否为空
			...
			header('Location: http://www.wjtr.com/index.php?module=default&submodule=order&controller=pay&action=paysuccess');
		}
	}
	...
	function notify() {
		...
		if ($this->notify_result) { //验证成功
			...
			if ($result_pay == "SUCCESS") {
				...
				//——请根据您的业务逻辑来编写程序——
				/**
				 * 1: 修改订单状态,付款时间，确认时间和订单详情状态
				 * 2： 修改交易状态 和 连连支付单号
				 * 3: 记录附件付款记录
				 * 4: 发送 申请路演付款成功的通知(假如是路演申请付款的话)
				 */
				
				$order_info = Order::find('order_no = ?', $no_order)->query();
				$order_info->status = 1;
				$order_info->confirm_time = time();
				$order_info->pay_time = time();
				$order_info->save();
				
				$orderdetail_info = Orderdetail::find('order_id = ?', $order_info->id)->query();
				$orderdetail_info->status = 1;
				$orderdetail_info->save();
				
				$orderpay_info = Pay::find('order_id = ?', $order_info->id)->query();
				
				$orderpay_info->status = 1;
				$orderpay_info->llpay_oid = $oid_paybill;
				$orderpay_info->save();
				
				if ($orderdetail_info->model == 'Roadshowapply') {
					
					$Roadshowapply = Roadshowapply::find('id = ?', $orderdetail_info->post_id)->query();
					$Roadshowapply->status = 2;
					$Roadshowapply->save();
					
					//发送通知
					$notice_data['model'] = 'Roadshowapply';
					$notice_data['post_id'] = $orderdetail_info->post_id;
					$notice_data['uid'] = $order_info->uid;
					$notice_data['type'] = 'roadshow_apply_pay';
					
					$notice_info = Notice::find($notice_data)->query();
					
					if (!$notice_info->id) {
						$notice_data['title'] = '<h5 class="black">您申请的路演已付款成功：</h5><div class="dark mt5">
									感谢您的支持<br>
									我们会将服务做到更好<br>
								</div>';
					...
					$data['attachment_id'] = $order_info->orderDetail->post_id;
					$data['pay_id'] = $order_info->orderPay->id;
					$data['created'] = time();
					$data['price'] = $order_info->orderDetail->price;
					$data['amount'] = $order_info->orderDetail->amount;
					$data['total_fee'] = $order_info->orderDetail->total_fee;
					
					$pay = new Attachmentpay($data);
					$pay->save();
				}
			}
	...
	function buildRequestMysign($para_sort) {
		...
		$prestr = Helper_Helperllpay::createLinkstring($para_sort);
		$mysign = "";
		switch (strtoupper(trim($this->_config['sign_type']))) {
			case "MD5":
				$mysign = Helper_Helperllpay::md5Sign($prestr, $this->_config['key']);
				break;
			default:
				$mysign = "";
		}
	...
	function buildRequestPara($para_temp) {
		//除去待签名参数数组中的空值和签名参数
		$para_filter = Helper_Helperllpay::paraFilter($para_temp);
		//对待签名参数数组排序
		$para_sort = Helper_Helperllpay::argSort($para_filter);
		//生成签名结果
	...
	function buildRequestParaToString($para_temp) {
		...
		$request_data = Helper_Helperllpay::createLinkstringUrlencode($para);
...
// 	}
	...
	function buildRequestHttp($para_temp) {
		...
		//远程获取数据
		$sResult = Helper_Helperllpay::getHttpResponsePOST($this->llpay_gateway_new, $this->_config['cacert'], $request_data, trim(strtolower($this->_config['input_charset'])));
	...
	function buildRequestHttpInFile($para_temp, $file_para_name, $file_name) {
		...
		//远程获取数据
		$sResult = Helper_Helperllpay::getHttpResponsePOST($this->llpay_gateway_new, $this->_config['cacert'], $para, trim(strtolower($this->_config['input_charset'])));
	...
	function verifyNotify() {
		//生成签名结果
		$is_notify = true;
		$json = new Helper_Helperjson();
		$str = file_get_contents("php://input");
		
		$val = $json->decode($str);
		
		file_put_contents("log.txt", $str . "\n", FILE_APPEND);
		$oid_partner = Helper_Helperllpay::getJsonVal($val, 'oid_partner');
		$sign_type = Helper_Helperllpay::getJsonVal($val, 'sign_type');
		$sign = Helper_Helperllpay::getJsonVal($val, 'sign');
		$dt_order = Helper_Helperllpay::getJsonVal($val, 'dt_order');
		$no_order = Helper_Helperllpay::getJsonVal($val, 'no_order');
		$oid_paybill = Helper_Helperllpay::getJsonVal($val, 'oid_paybill');
		$money_order = Helper_Helperllpay::getJsonVal($val, 'money_order');
		$result_pay = Helper_Helperllpay::getJsonVal($val, 'result_pay');
		$settle_date = Helper_Helperllpay::getJsonVal($val, 'settle_date');
		$info_order = Helper_Helperllpay::getJsonVal($val, 'info_order');
		$pay_type = Helper_Helperllpay::getJsonVal($val, 'pay_type');
		$bank_code = Helper_Helperllpay::getJsonVal($val, 'bank_code');
		$no_agree = Helper_Helperllpay::getJsonVal($val, 'no_agree');
		$id_type = Helper_Helperllpay::getJsonVal($val, 'id_type');
		$id_no = Helper_Helperllpay::getJsonVal($val, 'id_no');
		$acct_name = Helper_Helperllpay::getJsonVal($val, 'acct_name');
	...
	function getSignVeryfy($para_temp, $sign) {
		...
		switch (strtoupper(trim($this->_config['sign_type']))) {
			case "MD5":
				$isSgin = Helper_Helperllpay::md5Verify($prestr, $sign, $this->_config['key']);
				break;
			default:
				$isSgin = false;
		}
